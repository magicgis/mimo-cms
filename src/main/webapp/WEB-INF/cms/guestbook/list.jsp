...
<ul>
  ...
  <div class="clear"></div>
   ...
   <!--信息搜索 start-->
    <div class="search"><ul>
    <li>名称： <input type="text" name="params[email]" value="${page.params.email }" class="input1" /></li>
   <li>
  ...
  <table cellpadding="1" cellspacing="1"  class="table_bj">
    ...
    <c:choose>
		<c:when test="${not empty page.result}">
			<c:forEach items="${page.result}" var="guestbook" begin="0">
				<tr class="table_con">
					<td><input type="checkbox" name="items" id="checkbox" value="${guestbook.id}"/></td>
					<td>${guestbook.email}&nbsp;</td>
					<td>${guestbook.content}&nbsp;</td>
					<td class="date">
						${guestbook.createTime}&nbsp;
					</td>
					<td>
						<shiro:hasPermission name="guestbook:edit">
